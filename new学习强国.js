 ...
 */
// 基础数据
var { delay_time } = hamibot.env;//跳转页面加载的时间
delay_time = Number(delay_time) * 1000;
var { whether_improve_accuracy } = hamibot.env;//是否提高四人赛双人对战正确率
var { all_weekly_answers_completed, all_special_answer_completed } = hamibot.env;//之前的每周答题是否全部完成、之前的专项答题是否全部完成
var { whether_complete_subscription } = hamibot.env;//是否完成订阅模块
var { whether_complete_speech } = hamibot.env;//是否完成发表言论模块
var { sct_token, pushplus_token } = hamibot.env;//server酱、微信消息推送
var { whether_mute } = hamibot.env;//是否在脚本执行期间静音
var { whether_froze_app } = hamibot.env;//是否启用冻结学习强国
